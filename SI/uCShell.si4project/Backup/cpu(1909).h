...
// 不能在中断服务内使用.
#define USER_CPU_FREERTOS_INTERRUPT_MODE            FREERTOS_INTERRUPT_MODE     /* 在这里为FREERTOS中断模式 */

#if     ( USER_CPU_FREERTOS_INTERRUPT_MODE == FREERTOS_INTERRUPT_MODE)

#include "FreeRTOS.h"

#define  CPU_SR_ALLOC()             

//#define  CPU_CRITICAL_ENTER()           portENTER_CRITICAL()          /* Disable   interrupts.                          */
//#define  CPU_CRITICAL_EXIT()            portEXIT_CRITICAL()          /* Re-enable interrupts.                           */
